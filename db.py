...
################################################################################
################################################################################
#//CRIAR / CONECTAR / LIMPAR//# [Cria e prepara o Banco para receber os dados]
################################################################################
################################################################################
################################################################################
...
def conectar_banco():
  #  """Conecta ao banco de dados SQLite"""
    try:     
        ...
        print('#########[PROJETO JIU-JITSU]#############')
        print("\033[32m-----BANCO DE DADOS CRIADO E CONECTADO ------\033[0m")
        print('#########//////////////////#############')
...
def limpar_dados(cur):
    ...
    print("\033[32mLimpando os dados...\033[0m")
   
    # Limpar os dados das tabelas considerando a ordem das foreign keys
    ...
    cur.execute("DELETE FROM sqlite_sequence WHERE name IN ('Atleta', 'Categoria', 'Lutas', 'Academia', 'Inscricoes');")
...
def inserir_categorias(cur, conectar):
...
def inserir_academia(cur, conectar):
...
def inserir_atetlas(cur, conectar):
...
def mostrar_categorias(cur):
    """Exibe todos os registros da tabela Categoria"""
    cur.execute("SELECT * FROM Categoria;")
    categorias = cur.fetchall()
    
    if not categorias:
        print("\033[93mNenhuma categoria encontrada.\033[0m")  # amarelo
    else:
        print("\033[92m=== CATEGORIAS CADASTRADAS ===\033[0m")  # verde
        for cat in categorias:
            print(f"ID: {cat['id_peso']} | Categoria: {cat['categoria_peso']} | Limite: {cat['limite_peso']} | Sexo: {cat['sexo']}")

def mostrar_academias(cur):
    """Exibe todos os registros da tabela Academia"""
    cur.execute("SELECT * FROM Academia;")
    categorias = cur.fetchall()
    
    if not categorias:
        print("\033[93mNenhuma Academia encontrada.\033[0m")  # amarelo
    else:
        print("\033[92m=== ACADEMIAS CADASTRADAS ===\033[0m")  # verde
        for cat in categorias:
            print(f"ID: {cat['id_academia']} | Nome: {cat['nome_academia']} | CNPJ: {cat['CNPJ']} | telefone: {cat['telefone']}")

def mostrar_atletas(cur):
    """Exibe todos os registros da tabela Atletas"""
...
def start():
    ...
    try:
        ...
        limpar_dados(cur)
        ## 2 - Etapa
        inserir_categorias(cur,conectar)
        mostrar_categorias(cur)
        inserir_academia(cur, conectar)
        mostrar_academias(cur)
        inserir_atetlas(cur, conectar)
        mostrar_atletas(cur)
